...
/**Single выпускают или событие об успешном завершении (success) или об ошибки (error). События типа
...
/**Completable выпускает только событие о завершение или ошибке.Такой издатель не выпускает никаких
...
/**Maybe сочетание Single т Completable. Он может выпускать события success, complete и error. Если вам
нужно реализовать операцию, которая может быть либо успешным, либо неудачным, и при желании вернуть
значение в случае успеха. Пример чтение данных из базы. onSuccess() для вернувшихся данных, onError - для
ошибки, onComplete() если база вернула пустоту. https://umbrellait.rapira.com/du1T4L */
// TODO: Реализовать пример с пустым результатом, чтобы не падала цепочка и другое решение данной проблемы - Resource extension
fun maybeExample() {
